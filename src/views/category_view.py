...
class CategoryView(tk.Toplevel):
    def __init__(self, parent, category: Category, controller, role='user', refresh_callback=None):
        super().__init__(parent)
        self.title(f"Chi tiết danh mục: {category.category_name}")
    ...
    def setup_ui(self):
        ...
        tk.Label(main_frame, text="Tên danh mục:", bg="#f5f5f5", font=("Arial", 10, "bold")).grid(row=1, column=0,
                                                                                                  sticky="w", pady=5)
        self.entry_name = tk.Entry(main_frame, width=40)
        self.entry_name.grid(row=1, column=1, sticky="we", pady=5)
        tk.Label(main_frame, text="Mô tả:", bg="#f5f5f5", font=("Arial", 10, "bold")).grid(row=2, column=0, sticky="nw",
                                                                                           pady=5)
        self.txt_desc = tk.Text(main_frame, height=5, width=30, relief="solid", bd=1)
        self.txt_desc.grid(row=2, column=1, sticky="we", pady=5)
    ...
    def load_data(self):
        self.entry_id.insert(0, str(self.category.category_id))
        self.entry_name.insert(0, self.category.category_name)
        self.txt_desc.insert("1.0", self.category.category_des if self.category.category_des else "")
    ...
    def save_changes(self):
        ...
        if not new_name:
            messagebox.showwarning("Cảnh báo", "Tên danh mục không được để trống", parent=self)
            return
        self.category.category_name = new_name
        self.category.category_des = new_desc
        success, msg = self.controller.handle_update_category(self.category)
        if success:
